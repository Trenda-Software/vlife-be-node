...
export default class DataService {
    ...
    async initDBData() {
        ...
        const user1 = await UserModel.create({ dni: 24111222, name: 'Javier', surname: 'Hack', pwd: 'javi1234', email: 'javierhack@gmail.com' });
        await user1.setCountry(country1);
        await user1.setProvince(province1);

        const user2 = await UserModel.create({ dni: 24222111, name: 'Mariano', surname: 'Escudero', pwd: '$2a$10$2L0ZZ1D9glTYwSX/Zff..uPUhk/3XPak6/N8arA5Fk.E7FrNT7HHq', email: 'maca@gmail.com' });
        ...
        const professional2 = await ProfessionalModel.create({ name: 'Pedro', surname: 'Del Medico', in_service: false });
        await professional2.setCountry(country1);
        await professional2.setProvince(province1);
        const professional3 = await ProfessionalModel.create({ name: 'Pablo', surname: 'Del Hopitalet', in_service: false });
        await professional3.setCountry(country1);
        await professional3.setProvince(province1);
        const professional4 = await ProfessionalModel.create({ name: 'Juan', surname: 'Camillet', in_service: false });
