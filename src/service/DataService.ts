...
export default class DataService {
    ...
    async initDBData() {
        ...
        const professional2 = await ProfessionalModel.create({ name: 'Pedro', surname: 'Del Medico', in_service: false, on_line: true });
        await professional2.setCountry(country1);
        await professional2.setProvince(province1);
        const professional3 = await ProfessionalModel.create({ name: 'Pablo', surname: 'Del Hopitalet', in_service: false, on_line: true });
        await professional3.setCountry(country1);
        await professional3.setProvince(province1);
        const professional4 = await ProfessionalModel.create({ name: 'Juan', surname: 'Camillet', in_service: false, on_line: true });
        await professional4.setCountry(country1);
        await professional4.setProvince(province1);
        const professional5 = await ProfessionalModel.create({ name: 'Maria', surname: 'DeGuardia', in_service: false, on_line: true });
