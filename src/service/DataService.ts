...
export default class DataService {
    ...
    async cancelRequestsByTime(interval: any) {
        setInterval(async () => {
            ...
            const requests = await this.dbClient.query(
                "update Requests set commentprof = 'Cancelado por no respuesta del profesional',staterequest=9 where (timestampdiff(minute,  Requests.updatedAt,now() ) > " +
                process.env.CANCEL_REQUEST_MINUTES +
                ' )  and staterequest = 0 and id <> -1',
    ...
    async initDBData() {
        ...
        /*         const province1 = await ProvinceModel.create({ code: 'BSAS', name: 'Buenos Aires' });
                ...
                const vlifeparam = await VlifeParamModel.create({
                    comvlifeprof: 0.8,
                    plusxdistancia: 1.1,
                    plusxhorario: 1.1,
                    distanciaplus: 10,
