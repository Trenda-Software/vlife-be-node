                ...
                if (err) {
                    ...
                    try {
                        ...
                        for (let usuario in usuarios) {
                            ...
                            const servicios = await ds.dbClient.query("select Professionals.id,name,surname,sum(cost) as cost, comvlife, picture from PracticeCosts  inner join Professionals on Professionalid = Professionals.id where (" + preacticasID + ") and ProfessionalId = " + usuarios[usuario].ProfId + "  group by Professionalid", { type: Sequelize.QueryTypes.SELECT });
